...
(define (make-logger file)
  (let* ((logfd (if (string? file) (open file (bitwise-ior open/write open/append open/create)) #f))
    ...
    (define (log-error . o)
      (cond ((>= level 1)
             (let ((msg (apply show #f (make-datestring (current-seconds)) " ERROR " o)))
               (mutex-lock! mutex)
               (show logport msg nl)
               (log-flush)
               (mutex-unlock! mutex))))
    ...
    (define (log-warning . o)
      (cond ((>= level 2)
             (let ((msg (apply show #f (make-datestring (current-seconds)) " WARNING " o)))
               (mutex-lock! mutex)
               (show logport msg nl)
               (log-flush)
               (mutex-unlock! mutex))))
    ...
    (define (log-info . o)
      (cond ((>= level 3)
             (let ((msg (apply show #f (make-datestring (current-seconds)) " INFO " o)))
               (mutex-lock! mutex)
               (show logport msg nl)
               (log-flush)
               (mutex-unlock! mutex))))
    ...
    (define (log-debug . o)
      (cond ((>= level 4)
             (let ((msg (apply show #f (make-datestring (current-seconds)) " DEBUG " o)))
               (mutex-lock! mutex)
               (show logport msg nl)
               (log-flush)
               (mutex-unlock! mutex))))
